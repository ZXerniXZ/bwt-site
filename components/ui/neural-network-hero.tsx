...
function ShaderPlane() {
  ...
  const CustomShaderMaterial = 'cPPNShaderMaterial' as any;
...
export function ShaderBackground() {
  ...
  return (
    <div className="fixed inset-0 z-[-10] w-full h-full pointer-events-none bg-black" aria-hidden>
      <Canvas
        camera={camera}
...
export default function Hero({
  ...
  badgeLabel = "New",
  ctaButtons = [
    { text: "Inizia", href: "#visualizer", primary: true },
    { text: "La Storia", href: "#history" }
  ],
  microDetails = ["BWT Optimization", "Data Science", "Genomics"]
}: HeroProps) {
  ...
  useGSAP(
    () => {
      ...
      gsap.set([badgeRef.current, headerRef.current, paraRef.current, ctaRef.current], {
          autoAlpha: 0,
          y: 30,
          filter: 'blur(10px)'
      });
      tl.to(badgeRef.current, { autoAlpha: 1, y: 0, filter: 'blur(0px)', duration: 0.6 })
        .to(headerRef.current, { autoAlpha: 1, y: 0, filter: 'blur(0px)', duration: 0.8 }, "-=0.4")
        .to(paraRef.current, { autoAlpha: 1, y: 0, filter: 'blur(0px)', duration: 0.8 }, "-=0.6")
  ...
  return (
    <section ref={sectionRef} className="relative h-screen w-full flex items-center justify-center pb-32">
      <div className="relative mx-auto flex max-w-7xl flex-col items-start justify-center gap-6 px-6 md:px-10 lg:px-16 w-full z-30 text-center md:text-left items-center md:items-start">
        ...
        <h1 ref={headerRef} className="max-w-4xl text-5xl font-extralight leading-[1.05] tracking-tight text-white sm:text-7xl md:text-8xl lg:text-9xl drop-shadow-2xl">
          {title}
        </h1>

        <p ref={paraRef} className="max-w-xl text-base font-light leading-relaxed tracking-tight text-white/75 sm:text-lg md:text-xl">
          {description}
        </p>

        <div ref={ctaRef} className="flex flex-wrap items-center justify-center md:justify-start gap-3 pt-2">
          {ctaButtons.map((button, index) => (
            <a
              key={index}
              href={button.href}
              className={`rounded-2xl border border-white/10 px-6 py-3.5 text-sm font-light tracking-tight transition-all duration-300 ${
                button.primary
        ...
        <ul className="mt-8 flex flex-wrap justify-center md:justify-start gap-6 text-xs font-extralight tracking-tight text-white/60">
